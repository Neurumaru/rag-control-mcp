...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def _cleanup_old_entries(self, current_time: float) -> None:
        ...
        for client_ip in list(self.client_history.keys()):
            self.client_history[client_ip] = [
                req_time for req_time in self.client_history[client_ip] if req_time > cutoff_time
            ]
    ...
    def _is_rate_limited(self, client_ip: str, current_time: float) -> bool:
        ...
        recent_requests = [req_time for req_time in requests if req_time > current_time - 10]
