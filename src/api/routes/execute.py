...
async def execute_query(request: QueryRequest):
    try:
        ...
        try:
            ...
            logger.info(
                f"Successfully executed pipeline '{pipeline.name}' with execution ID {execution.id}"
            )
...
async def list_executions(pipeline_id: str = None, status: str = None, limit: int = 50):
    try:
        # Convert parameters
        pipeline_uuid = None
        if pipeline_id:
        ...
        executions = pipeline_registry.list_executions(
            pipeline_id=pipeline_uuid, status=status_filter, limit=limit
        )
