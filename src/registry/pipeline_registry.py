...
from ..models.pipeline import Pipeline, PipelineExecution, PipelineStatus
...
class PipelineRegistry:
    ...
    async def unregister_pipeline(self, pipeline_id: UUID, force: bool = False) -> None:
        ...
        async with self._lock:
            ...
            running_executions = [
                exec_id
                for exec_id in self._pipeline_executions.get(pipeline_id, set())
    ...
    async def create_execution(
        self, pipeline_id: UUID, input_data: Dict = None
    ) -> PipelineExecution:
    ...
    async def fail_execution(
        self, execution_id: UUID, error_message: str, error_step_id: UUID = None
    ) -> None:
    ...
    def get_execution_stats(self, pipeline_id: Optional[UUID] = None) -> Dict[str, any]:
        ...
        if completed_with_time:
            avg_execution_time = sum(e.execution_time_seconds for e in completed_with_time) / len(
                completed_with_time
            )
        ...
        return {
            ...
            "success_rate": (
                completed_executions / total_executions if total_executions > 0 else 0
            ),
    ...
    def get_pipeline_stats(self) -> Dict[str, any]:
        ...
        active_pipelines = len(
            [p for p in self._pipelines.values() if p.status == PipelineStatus.ACTIVE]
        )
        inactive_pipelines = len(
            [p for p in self._pipelines.values() if p.status == PipelineStatus.INACTIVE]
        )
        error_pipelines = len(
            [p for p in self._pipelines.values() if p.status == PipelineStatus.ERROR]
        )
        ...
        return {
            ...
            "average_steps_per_pipeline": (
                total_steps / total_pipelines if total_pipelines > 0 else 0
            ),
        }
    ...
    async def cleanup_old_executions(self, max_age_days: int = 30, max_count: int = 1000) -> int:
        ...
        async with self._lock:
            ...
            for execution in self._executions.values():
                ...
                if (
                    execution.started_at.timestamp() < cutoff_date
                    or len(self._executions) > max_count
                ):
