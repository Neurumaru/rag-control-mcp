...
async def list_pipelines(
    ...
    try:
        ...
        pipelines_list = registry.list_pipelines(
            status=status_filter, category=getattr(filters, "category", None)
        )
...
async def get_pipeline(pipeline_id: str):
    try:
        ...
        return PipelineResponse(pipeline=pipeline, message="Pipeline retrieved successfully")
    except ValueError:
...
async def update_pipeline(pipeline_id: str, request: PipelineUpdateRequest):
    try:
        ...
        return PipelineResponse(pipeline=updated_pipeline, message="Pipeline updated successfully")
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid pipeline ID format")
    except Exception as e:
