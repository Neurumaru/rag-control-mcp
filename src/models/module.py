...
class DataSchema(BaseModel):
    ...
    format: str = Field(
        ..., description="Specific format (e.g., 'json', 'plain_text', 'vector_array')"
    )
    ...
    distance_metric: Optional[str] = Field(
        None, description="Distance metric for vector operations"
    )
    ...
    schema_definition: Optional[Dict[str, Any]] = Field(
        None, description="JSON schema or structure definition"
    )
...
class ModuleCapabilities(BaseModel):
    ...
    supports_batch: bool = Field(
        default=True, description="Whether module supports batch processing"
    )
    ...
    supports_fallback: bool = Field(
        default=False, description="Whether module has fallback behavior"
    )

    # Monitoring
    metrics_collected: List[str] = Field(
        default_factory=list, description="Metrics this module collects"
    )
...
class Module(BaseModel):
    ...
    capabilities: Optional[ModuleCapabilities] = Field(
        None, description="Module transformation capabilities"
    )
...
class ModuleRegistrationRequest(BaseModel):
    ...
    dependencies: List[str] = Field(
        default_factory=list, description="Dependency module IDs as strings"
    )
    tags: List[str] = Field(default_factory=list)
